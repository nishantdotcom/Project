...
export default function CatSprite({ movement, coOrdinate, greetWithTime }) {
  const [valid, setiSvalid] = useState(false);
  const [time, settime] = useState(parseInt(greetWithTime.time));
  console.log(greetWithTime);

  console.log(time, "time");
  if (time) {
    console.log("time inds", time);
    const setid = setTimeout(() => {
      console.log("indide valid", valid);
  ...
  return (
    <div className=" ">
      {movement.text != "" && movement.duration == 0 && (
        <>
          <div className="  flex justify-center w-40 border border-black p-2 rounded-lg">
            {movement.text}
          </div>
        </>
      )}
